...
class Learner(nn.Module):
    def __init__(
        ...
        self.checkpoint_every_n_steps = checkpoint_every_n_steps if checkpoint_every_n_steps != -1 else None
            ...
            with tqdm(initial=self.step_idx, total=self.train_iters) as pbar_steps:
                while self.step_idx < self.train_iters:
                    ...
                    for batch_idx, batch in enumerate(
                        ...
                        if (self.checkpoint_every_n_steps is not None) and (
                            self.step_idx % self.checkpoint_every_n_steps == 0
                            and self.step_idx > 0
                        ):
                            self.save_checkpoint(
